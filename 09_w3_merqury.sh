...
#SBATCH --output=/data/users/trakic/assembly_annotation_course/output_err/output_merqury_%j.o
#SBATCH --error=/data/users/trakic/assembly_annotation_course/output_err/error_merqury_%j.e

WORKDIR=/data/users/trakic/assembly_annotation_course/
READS=/data/users/trakic/assembly_annotation_course/RRS10/ERR11437326.fastq.gz
OUTDIR=$WORKDIR/results/week3_merqury
CONTAINER_PATH="/containers/apptainer/merqury_1.3.sif"

mkdir -p $OUTDIR

FLYE=$WORKDIR/for_busco/flye_assembly.fasta
HIFIASM=$WORKDIR/for_busco/hifisam.asm.bp.p_ctg.fa
LJA=$WORKDIR/for_busco/ija_assembly.fasta
